...
const Remittances = () => {
  ...
  const [periodTab, setPeriodTab] = useState("yearly"); // "monthly" | "yearly" | "allTime"
  ...
  const fetchData = async (
    ...
    try {
      ...
      if (period !== "allTime") params.year = y;
  ...
  useEffect(() => {
    fetchData(year, relFilter, periodTab);
  }, [year, relFilter, filterPayment, periodTab]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setShowForm(false);
      setEditing(null);
      fetchData(year, relFilter, periodTab, { silent: true });
    } catch (err) {
  ...
  const doDelete = async () => {
    ...
    try {
      if (confirmDel === "all") {
        ...
        const { data } = await api.delete(`/remittances/${confirmDel}`);
        toast.success(data.message || t("success"));
      }
      setConfirmDel(null);
      fetchData(year, relFilter, periodTab, { silent: true });
  ...
  const handleExport = () => {
    try {
      ...
      XLSX.writeFile(workbook, `remittances_${Date.now()}.xlsx`);
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const res = await api.post("/remittances/import", {
          items: parsedItems,
        });
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="relative">
              ...
              <input
                ...
                placeholder={`${t("recipient")}, ${t("note")}…`}
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-slate-200/80 dark:border-slate-700/80 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                  ...
                  <th className="p-4">{t("relation")}</th>
                ...
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0 space-y-0.5">
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium truncate">
                      {item.recipient} • {item.paymentMethod || "N/A"} •{" "}
                      {new Date(item.remittanceDate).toLocaleDateString()}
                    </p>
                  </div>
                </div>
